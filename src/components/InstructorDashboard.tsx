...
export default function Instructor({userLoginUtility}: {userLoginUtility: any}) {
    
  // Setting the interface for instructorData
    ...
    const [instructorData, setInstructorData] = React.useState<any>(null); // Initialize as null
    ...
    <ThemeProvider theme={defaultTheme}>
      <Box sx={{ display: 'flex' }}>
        ...
        <AppBar position="absolute" open={open}>
          ...
          >
            ...
            <Typography
              ...
              sx={{ flexGrow: 1 }}
            >
               Welcome {instructorData.instructorData.name}
            </Typography>
          </Toolbar>
        </AppBar>
        <Drawer variant="permanent" open={open}>
          ...
          > 
            <Button style={{ width: '100%', color: 'rgba(0,0,0,0.5)'}} onClick= {() => setComponent('pending')}>
              {open ? <><AddTaskIcon/>-Pending Requests</> : <AddTaskIcon />}
            </Button>
          </Toolbar>
          {/* <Toolbar
            sx={{
          ...
          > 
            <Button style={{ width: '100%', color: 'rgba(0,0,0,0.5)'}} onClick= {() => setComponent('leave')}>
              {open ? <><AddCardIcon/>-Leave</> : <AddCardIcon />}
            </Button>
          </Toolbar> */}
        ...
        >
          ...
          <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
            ...
            {openUser ? <ViewUser user = {{userData: instructorData.instructorData,isOpen: openUser, role: 'course instructor'}}/> : null}
            {!openUser ? <>
              {component === 'pending' ?
                <PendingReq instructorData={instructorData}/>
              : component === 'course' ?
                <ViewCourses courses={instructorData.courseData} />
              : component === 'leave' ?
              <></>: null}
            </> : null}
            </Grid>
